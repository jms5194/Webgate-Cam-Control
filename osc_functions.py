...
class WebgateOSCReceiver:
    ...
    def build_webgate_osc_servers(self):
        self.webgate_client = udp_client.ThreadingOSCUDPServer(settings.remote_ip, settings.send_port)
        self.webgate_dispatcher = dispatcher.Dispatcher()
        self.receive_webgate_osc()
        try:
            self.webgate_osc_server = osc_server.ThreadingOSCUDPServer((self.find_local_ip_in_subnet(settings.remote_ip),
                                                                       settings.receive_port), self.webgate_dispatcher)
            print("Webgate OSC Server Started")
            self.webgate_osc_server.server_forever()
    ...
    def osc_button_handler(self, osc_address, button_pressed):
